...
import { jobCategory, jobType, location, experienceLevel, category } from '../../Data/data'; // Ensure this path is correct
...
// Define props for the Siemens component
interface SiemensProps {
    selectedCompany: string;
}

const Siemens: React.FC<SiemensProps> = ({ selectedCompany }) => {
    ...
    const fetchJobs = async () => {
        ...
        const queryParams = [
            ...
            experienceLevel && `level=${experienceLevelCode}`,
            jobCategoryCode && `family=${jobCategoryCode}`,
            category && `is_remote=${categoryCode}`,
            jobTypeCode && `fulltime_or_parttime=${jobTypeCode}`,
            `start=${(currentPage - 1) * resultsPerPage}`,
            `num=${resultsPerPage}`,
        ].filter(Boolean).join('&');

        const url = `/siemens/api/apply/v2/jobs?domain=siemens.com&${queryParams}`;
        console.log(url);
    ...
    const fetchJobDetails = async (jobId: string) => {
        ...
        const aiResumeUrl = `/siemens/api/apply/v2/jobs/${jobId}?domain=siemens.com`;
    ...
    }, [jobCategoryCode, jobTypeCode, locationCode, experienceLevelCode, categoryCode, currentPage]);
    ...
    return (
        <div className="p-4">
            {/* Filters integrated directly in Siemens */}
